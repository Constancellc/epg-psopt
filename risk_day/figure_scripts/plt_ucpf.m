...
for i = 1:numel(k)
    UB = [k(i) 1 (2-k(i))];
%     UB = [k(i) (1.5-k(i)/2)*[1 1] ];
    
    DSSCircuit = set_loads_ub(DSSCircuit,S0*UB);
    DSSSolution.Solve;
    ...
    for j = 1:size(vc,2)
        cvf(j,i) = norm(vc(:,j) - YNodeV(4:end))/norm(YNodeV(4:end));
        clf(j,i) = abs(Sls(j,i) - sl(i))/abs(sl(i));
    end
...
%%
fg_nm = [fg_lc,'lin_cvf_a'];
fig = figure('Color','White','Position',fg_ps);

plot(k,cvf);
xlabel('$k_{A}$'); ylabel('$||\tilde{v} - v||_{2}/||v||_{2}$');
...
%%
fg_nm = [fg_lc,'lin_clf_a'];
fig = figure('Color','White','Position',fg_ps);

plot(k,clf);
xlabel('$k_{A}$'); ylabel('$|\tilde{S}_{l} - S_{l}|/|S_{l}|$');
lgnd=legend('$\hat{S} = 0.2 \times S_{Ld}$','$\hat{S} = 0.6 \times S_{Ld}$',...
            '$\hat{S} = 1.0 \times S_{Ld}$','$\hat{S} = 1.4 \times S_{Ld}$');%
set(lgnd,'Interpreter','Latex')
xlim([-inf inf]);
...
% 
% %%
% fg_nm = [fg_lc,'lin_cvf_b'];
% fig = figure('Color','White','Position',fg_ps);
% 
% plot(k,cvf);
% xlabel('$k_{B}$'); ylabel('$||\tilde{v} - v||_{2}/||v||_{2}$');
% lgnd=legend('$\hat{S} = 0.2 \times S_{Ld}$','$\hat{S} = 0.6 \times S_{Ld}$',...
%             '$\hat{S} = 1.0 \times S_{Ld}$','$\hat{S} = 1.4 \times S_{Ld}$');%
% set(lgnd,'Interpreter','Latex')
% xlim([-inf inf]);
...
% 
% %%
% fg_nm = [fg_lc,'lin_clf_b'];
% fig = figure('Color','White','Position',fg_ps);
% 
% plot(k,clf);
% xlabel('$k_{B}$'); ylabel('$|\tilde{S}_{l} - S_{l}|/|S_{l}|$');
% lgnd=legend('$\hat{S} = 0.2 \times S_{Ld}$','$\hat{S} = 0.6 \times S_{Ld}$',...
%             '$\hat{S} = 1.0 \times S_{Ld}$','$\hat{S} = 1.4 \times S_{Ld}$');%
% set(lgnd,'Interpreter','Latex')
% xlim([-inf inf]);
