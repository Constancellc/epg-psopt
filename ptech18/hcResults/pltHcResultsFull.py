...
if 'f_mcLinCmp' in locals():
    ...
    for x in X:
        if x in Xused:
            ...
            ii += 1
        i+=1
    ax.plot(0,0,'-',color=clrH,label='OpenDSS (Nominal Bandwidth)')
    ax.plot(0,0,'-',color=clrB,label='OpenDSS (Static Bandwidth)')
    plt.legend(fontsize='small')
...
if 'f_plotLpUpg' in locals():
    ...
    for x in Xlp:
        ...
        plt.yticks([5,4,3,2,1],['No T or Q','T, no Q (cntrl)','T and Q (cntrl)','Nom. (local)','Upg. (local)'])
        plt.xlim((-2,102))
        plt.grid(True)
        plt.xlabel('Loads with PV installed, %')
