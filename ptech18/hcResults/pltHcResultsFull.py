...
for rslt in rsltsFrac.values():
    ...
    dataSet.append('%.2f' %  (np.mean(np.abs(rslt['dssHcRslTgt']['Vp_pct']-rslt['linHcRsl']['Vp_pct']))) )
    dataSet.append('%.2f' %  (np.mean(np.abs(rslt['dssHcRslNom']['Vp_pct']-rslt['linHcRsl']['Vp_pct']))) )
    timeTableData.append(dataSet)
    
    dataSet = []
    dataSet.append(feedersTidy[rslt['feeder']])
    dataSet.append(feedersTidy[rslt['feeder']]) # <--- to do!
    dataSet.append(feedersTidy[rslt['feeder']])
    dataSet.append(feedersTidy[rslt['feeder']])
...
if 't_rsltSvty' in locals():
    ...
    label='rsltSvty'
    heading = ['Model','Nmc error','Preconditioning error','Preconditioning avoided cost']
    data = rsltSvtyData
    basicTable(caption,label,heading,data,TD)
# ===============================================
