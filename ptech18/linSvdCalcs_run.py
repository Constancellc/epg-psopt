...
# pdfSolar = {'8500node':0.21,'epri5':0.35,'epri7':0.34,'epriJ1':0.17,'epriK1':2.35,'epriM1':1.5,'epri24':1.08}

# # pdf.pdf['clfnSolar'] = {'k':4.21423544,'th_kW':1.16}
# # pdf.pdf['clfnSolar'] = {'k':4.21423544,'th_kW':pdfSolar[fdrs[fdr_i]]}

# LM.runLinHc(nMc,pdf.pdf,model='nom') # model options: nom / std / cor / mxt ?
# # print(LM.linHcRsl['Vp_pct'])
# plotCns(pdf.pdf['prms'],LM.linHcRsl['Cns_pct'][:,0,:],feeder=LM.feeder)
...
# # ============================ Choosing which to use (50, 100, 200% of californian solar)
# fdr_i_set = [5,6,8,9,0,14,17,18,22,19,20,21]
# th_kW_mult = {}
# for fdr_i in fdr_i_set:
    # print('Load Linear Model feeder:',fdrs[fdr_i],'\nPdf type:',pdfName,'\n',time.process_time())
    ...
    # LM.loadNetModel(LM.netModelNom)
    # pdfName = 'gammaFrac'; prms=np.array([0.05,0.75])
    # pdf = hcPdfs(LM.feeder,netModel=LM.netModelNom,pdfName=pdfName,prms=prms )
    # LM.runLinHc(nMc,pdf.pdf,model='nom') # model options: nom / std / cor / mxt ?
    # print(LM.linHcRsl['Vp_pct'])
    # if LM.linHcRsl['Vp_pct'][0]>90.:
        # th_kW_mult[fdrs[fdr_i]] = 0.5
    # elif LM.linHcRsl['Vp_pct'][1]<10.:
        # th_kW_mult[fdrs[fdr_i]] = 2.
    # else:
